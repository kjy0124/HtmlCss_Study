...
// 여기에 코드를 작성하세요
const filteredUser = users.filter((user) => user.age >= 25 && user.age < 35);
// 필터링된 사용자 중에서 이메일 주소가 gmail.com 도메인을 사용하는 사용자만을 추출하여 새로운 배열 생성하세요
// 여기에 코드를 작성하세요
const userEmail = filteredUser.filter((user) => user.email.endsWith('@gmail.com'));
...
renderUserData(userEmail); // 사용자 데이터를 그리는 함수 실행
